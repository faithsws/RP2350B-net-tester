...
static rt_err_t pico_uart_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    /*
     * RP2350B：部分引脚 UART TX/RX 在 F11(UART_AUX)，F2(UART) 反而是 CTS/RTS。
     * 例如 GPIO46/47：F2=CTS/RTS，F11=TX/RX。必须用 UART_FUNCSEL_NUM()。
     */
    gpio_set_function(uart->rx_pin, UART_FUNCSEL_NUM(uart->instance, uart->rx_pin));
    gpio_set_function(uart->tx_pin, UART_FUNCSEL_NUM(uart->instance, uart->tx_pin));
