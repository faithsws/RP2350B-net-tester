...
bi_decl(bi_1pin_with_name(BSP_SPI0_CS_PIN, "SPI0 CS"));
#endif
#ifdef BSP_USING_SPI1
bi_decl(bi_3pins_with_func(BSP_SPI1_MISO_PIN, BSP_SPI1_MOSI_PIN, BSP_SPI1_SCK_PIN, GPIO_FUNC_SPI));
bi_decl(bi_1pin_with_name(BSP_SPI1_CS_PIN, "SPI1 CS"));
#endif
...
static rt_err_t pico_spi_init(struct pico_spi *spi_drv, struct rt_spi_configuration *cfg)
{
    ...
    /* 必须先 spi_init（内部会 reset 并设默认 Mode0），再按需改 format */
    spi_init(spi_handle, cfg->max_hz);
    if (cfg->data_width != 8 || cpol != SPI_CPOL_0 || cpha != SPI_CPHA_0)
    {
    ...
    /*
     * CS 由 rt_pin / 设备层软件控制，切勿 gpio_init(CS)：
     * gpio_init 会清 OE，导致片选无法拉低，LCD 一直无显示。
     * 仅保证空闲高电平。
     */
    gpio_set_function(spi_drv->spi_cs_pin, GPIO_FUNC_SIO);
    gpio_set_dir(spi_drv->spi_cs_pin, GPIO_OUT);
    gpio_put(spi_drv->spi_cs_pin, 1);
...
{
    /* 单缓冲：仅发或仅收时 DMA 必须有合法地址，否则会写到 NULL 导致刷屏异常 */
    static uint8_t spi_dma_dummy;
    ...
    tx_buf = (message->send_buf != RT_NULL) ? (const uint8_t *)message->send_buf : &spi_dma_dummy;
    rx_buf = (message->recv_buf != RT_NULL) ? (uint8_t *)message->recv_buf : &spi_dma_dummy;

    c = dma_get_channel_config(spi->dma_tx);
    channel_config_set_read_increment(&c, message->send_buf != RT_NULL);
    channel_config_set_write_increment(&c, false);
    dma_channel_configure(spi->dma_tx, &c,
                          &spi_get_hw(spi->handle)->dr,
                          tx_buf,
                          message->length,
                          false);
    ...
    channel_config_set_read_increment(&c, false);
    channel_config_set_write_increment(&c, message->recv_buf != RT_NULL);
